...
namespace MLAPI.SceneManagement
{
    ...
    public static class NetworkSceneManager
    {
        ...
        private static async void OnSceneUnloadClient(Guid switchSceneGuid, Stream objectStream)
        {
            if (!NetworkingManager.Singleton.NetworkConfig.EnableSceneManagement || NetworkingManager.Singleton.NetworkConfig.UsePrefabSync)
            {
                ...
                using (PooledBitReader reader = PooledBitReader.Get(objectStream))
                {
                    ...
                    for (int i = 0; i < newObjectsCount; i++)
                    {
                        ...
                        NetworkedObject networkedObject = await SpawnManager.CreateLocalNetworkedObject(false, 0, prefabHash, parentNetworkId, position, rotation);
            ...
            else
            {
                ...
                using (PooledBitReader reader = PooledBitReader.Get(objectStream))
                {
                    ...
                    for (int i = 0; i < newObjectsCount; i++)
                    {
                        ...
                        NetworkedObject networkedObject = await SpawnManager.CreateLocalNetworkedObject(true, instanceId, 0, parentNetworkId, null, null);
                        SpawnManager.SpawnNetworkedObjectLocally(networkedObject, networkId, true, isPlayerObject, owner, objectStream, false, 0, true, false);
